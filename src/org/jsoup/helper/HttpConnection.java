...
/**
 * Implementation of {@link org.jsoup.Connection}.
...
public class HttpConnection implements Connection {
    ...
    public static class Response extends Base<Connection.Response> implements Connection.Response {
        ...
        private static HttpURLConnection createConnection(Connection.Request req) throws IOException {
           // Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("119.167.231.72", 8080));
            CookieHandler.setDefault(new CookieManager(null, CookiePolicy.ACCEPT_ALL));
            HttpURLConnection conn = null;
//            if (Jsoup.proxy!=null){
                conn = (HttpURLConnection) req.url().openConnection(Jsoup.proxy);
//            }else{
//                conn = (HttpURLConnection) req.url().openConnection();
//            }

           // conn.setRequestProperty("Cookie", Jsoup.session);
           conn.setReadTimeout(80000);
            conn.setConnectTimeout(80000);
            conn.addRequestProperty("User-Agent", "Mozilla/4.76");
//             conn.setRequestProperty("Referer", "http://www.webproxy.ca/");
