...
enum HtmlTreeBuilderState {
    ...
    BeforeHead {
        boolean process(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                tb.process(new org.jsoup.parser.Token.StartTag("head"));
                return tb.process(t);
            } else if (t.isEndTag()) {
                tb.error(this);
                return false;
    ...
    InHead {
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html")) {
                        ...
                        // org.jsoup special: update base the frist time it is seen
                        ...
                        // else if noscript && scripting flag = true: rawtext (org.jsoup doesn't run script, to handle as noscript)
                        tb.insert(start);
        ...
        private boolean anythingElse(org.jsoup.parser.Token t, TreeBuilder tb) {
            tb.process(new org.jsoup.parser.Token.EndTag("head"));
            return tb.process(t);
        }
    },
    InHeadNoscript {
        ...
        private boolean anythingElse(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            tb.error(this);
            tb.process(new org.jsoup.parser.Token.EndTag("noscript"));
            return tb.process(t);
        }
    },
    AfterHead {
        ...
        private boolean anythingElse(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            tb.process(new org.jsoup.parser.Token.StartTag("body"));
            tb.framesetOk(true);
            return tb.process(t);
        }
    },
            ...
            switch (t.type) {
                    ...
                    if (name.equals("html")) {
                        ...
                        for (int i = stack.size() - 1; i > 0; i--) {
                            ...
                            if (el.nodeName().equals("li")) {
                                tb.process(new org.jsoup.parser.Token.EndTag("li"));
                                break;
                            }
                        ...
                        for (int i = stack.size() - 1; i > 0; i--) {
                            ...
                            if (StringUtil.in(el.nodeName(), "dd", "dt")) {
                                tb.process(new org.jsoup.parser.Token.EndTag(el.nodeName()));
                                break;
                            }
                            if (tb.isSpecial(el) && !StringUtil.in(el.nodeName(), "address", "div", "p"))
                                break;
                        ...
                        if (tb.inButtonScope("button")) {
                            // close and reprocess
                            tb.error(this);
                            tb.process(new org.jsoup.parser.Token.EndTag("button"));
                            tb.process(startTag);
                        } else {
                            tb.reconstructFormattingElements();
                            tb.insert(startTag);
                        ...
                        if (tb.getActiveFormattingElement("a") != null) {
                            tb.error(this);
                            tb.process(new org.jsoup.parser.Token.EndTag("a"));
                        ...
                        if (tb.inScope("nobr")) {
                            tb.error(this);
                            tb.process(new org.jsoup.parser.Token.EndTag("nobr"));
                            tb.reconstructFormattingElements();
                        }
                        Element el = tb.insert(startTag);
                        tb.pushActiveFormattingElements(el);
                ...
                case EndTag:
                    ...
                    if (name.equals("body")) {
                        ...
                        boolean notIgnored = tb.process(new org.jsoup.parser.Token.EndTag("body"));
                        if (notIgnored)
                        ...
                        if (!tb.inButtonScope(name)) {
                            tb.error(this);
                            tb.process(new org.jsoup.parser.Token.StartTag(name)); // if no p to close, creates an empty <p></p>
                            return tb.process(endTag);
                        } else {
                        ...
                        tb.error(this);
                        tb.process(new org.jsoup.parser.Token.StartTag("br"));
                        return false;
                    } else {
                        return anyOtherEndTag(t, tb);
                    }
        ...
        boolean process(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            if (t.isCharacter()) {
                ...
                if (name.equals("caption")) {
                    ...
                    tb.process(new org.jsoup.parser.Token.StartTag("colgroup"));
                    return tb.process(t);
                    ...
                    tb.process(new org.jsoup.parser.Token.StartTag("tbody"));
                    return tb.process(t);
                } else if (name.equals("table")) {
                    tb.error(this);
                    boolean processed = tb.process(new org.jsoup.parser.Token.EndTag("table"));
            ...
            switch (t.type) {
                ...
                default:
                    if (tb.getPendingTableCharacters().size() > 0) {
                        for (org.jsoup.parser.Token.Character character : tb.getPendingTableCharacters()) {
    ...
    InCaption {
        boolean process(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            if (t.isEndTag() && t.asEndTag().name().equals("caption")) {
                ...
                tb.error(this);
                boolean processed = tb.process(new org.jsoup.parser.Token.EndTag("caption"));
                if (processed)
                    return tb.process(t);
    ...
    InColumnGroup {
        ...
        private boolean anythingElse(org.jsoup.parser.Token t, TreeBuilder tb) {
            boolean processed = tb.process(new org.jsoup.parser.Token.EndTag("colgroup"));
    ...
    InTableBody {
            ...
            switch (t.type) {
                case StartTag:
                    ...
                    if (name.equals("tr")) {
                        ...
                        tb.error(this);
                        tb.process(new org.jsoup.parser.Token.StartTag("tr"));
                        return tb.process(startTag);
        ...
        private boolean exitTableBody(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            ...
            tb.clearStackToTableBodyContext();
            tb.process(new org.jsoup.parser.Token.EndTag(tb.currentElement().nodeName())); // tbody, tfoot, thead
            return tb.process(t);
        }
    ...
    InRow {
        boolean process(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
            if (t.isStartTag()) {
                ...
                if (name.equals("tr")) {
                    ...
                    tb.process(new org.jsoup.parser.Token.EndTag("tr"));
                    return tb.process(t);
        ...
        private boolean handleMissingTr(org.jsoup.parser.Token t, TreeBuilder tb) {
            boolean processed = tb.process(new org.jsoup.parser.Token.EndTag("tr"));
            if (processed)
                return tb.process(t);
            else
                return false;
    ...
    InCell {
        ...
        private void closeCell(org.jsoup.parser.HtmlTreeBuilder tb) {
            if (tb.inTableScope("td"))
                tb.process(new org.jsoup.parser.Token.EndTag("td"));
            else
                tb.process(new org.jsoup.parser.Token.EndTag("th")); // only here if th or td in scope
        }
    },
    InSelect {
        boolean process(org.jsoup.parser.Token t, org.jsoup.parser.HtmlTreeBuilder tb) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html"))
                        ...
                        if (tb.currentElement().nodeName().equals("option"))
                            ...
                            tb.process(new org.jsoup.parser.Token.EndTag("optgroup"));
                        tb.insert(start);
                    } else if (name.equals("select")) {
                        tb.error(this);
                        return tb.process(new org.jsoup.parser.Token.EndTag("select"));
